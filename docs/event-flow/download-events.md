...
На текущем этапе `DownloadProgressChangedEvent` не является потоковым событием прогресса

Событие публикуется один раз после успешного завершения `downloadService.download(...)` и отражает
полностью завершенную загрузку

Промежуточный прогресс отдельных файлов не публикуется, потому что текущий контракт `DownloadService` не предоставляет
`DownloadFilesTask` потоковые обновления

Если появится требование отображать прогресс в UI во время загрузки, контракт загрузки может быть расширен
через listener/callback отдельной итерацией

Ошибки загрузки не публикуются специализированным событием. Они преобразуются в `Result.failure(...)` и передаются
вызывающему коду

Детали ошибки передаются через общий результат выполнения операции и общие события жизненного цикла `Operation`
