...
После `v0.8.0-presentation-launch-boundary` выбран узкий кандидат: локальная диагностика неожиданного сбоя принятого
presentation launch request

Сейчас application boundary передает безопасный `PresentationLaunchCompletion`, но техническая причина сбоя до получения
`LaunchResult` не сохраняется

Граница локальной диагностики зафиксирована в [ADR-0057](../decisions/records/ADR-0057-presentation-launch-diagnostics-boundary.md)

Следующая итерация реализует эту границу без введения общей политики логирования
