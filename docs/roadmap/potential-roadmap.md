...
До реализации общей политики логирования необходимо определить
...
Локальная диагностика неожиданных сбоев принятого presentation launch request реализована согласно
[ADR-0057](../decisions/records/ADR-0057-presentation-launch-diagnostics-boundary.md)

Она фиксирует источник сбоя и тип исключения, не передавая техническую причину в UI и не выводя исходное сообщение исключения

Это решение не вводит общую политику логирования, хранения диагностических данных или корреляции событий launcher lifecycle
...
Минимальный presentation launch flow завершен в milestone `v0.8.0-presentation-launch-boundary`

Дальнейшее развитие UI описано в [Пути развития presentation layer](presentation-roadmap.md) и требует отдельного подтвержденного сценария
...
## Воспроизводимый локальный запуск для проверки UI

Ручной запуск `launcher-ui` с manifest URI по умолчанию не обеспечивает воспроизводимого прохождения launcher lifecycle за
пределы загрузки манифеста

Для проверки нескольких presentation launch phases может понадобиться локальный сценарий с управляемым манифестом, временной
launcher directory и безопасной имитацией запуска игрового процесса

Отдельный runtime `dev mode` и специальные ветки в `LauncherEngine` пока не вводятся

Вернуться к направлению стоит, если возникнет потребность регулярно проверять визуальное прохождение нескольких фаз в
JavaFX и существующих автоматизированных тестов окажется недостаточно
